...
class NotificacaoService {
  ...
  async processarNotificacao({ tipo, usuarioId, emprestimoId, dados = {}, canal }) {
    logger.info('Processando notificação', { tipo, usuarioId, emprestimoId, canal });
    ...
    // Enviar por email (ou se canal === 'email')
    if (!canal || canal === 'email') {
      if (usuario.email) {
        ...
        resultado.canais.push({
          tipo: 'email',
          status: 'erro',
          erro: 'Usuário não possui email cadastrado'
        });
      }
    }

    // Enviar por SMS (ou se canal === 'sms')
    if (!canal || canal === 'sms') {
      if (dados.telefone) {
        try {
    ...
    if (resultado.canais.length === 0) {
      this.adicionarNaFila({ tipo, usuarioId, emprestimoId, dados, canal });
