  ...
  instance = {
    async emprestarLivro({ usuarioId, livroId }) {
      ...
      try {
        ...
        // Verificar se usuário existe (chamada HTTP)
        const userResponse = await fetch(`http://servico-usuarios:3001/usuarios/${usuarioId}`);
        if (!userResponse.ok) {
        ...
        // Atualizar cópias disponíveis via HTTP para serviço de livros
        await fetch(`http://servico-livros:3002/livros/${livroId}`, {
          method: 'PUT',
        ...
        // Notificar usuário (assíncrono - não bloquear resposta)
        fetch(`http://servico-notificacoes:3005/notificacoes`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            tipo: 'emprestimo',
    ...
    async devolverLivro({ emprestimoId }) {
      ...
      try {
        ...
        const livroResponse = await fetch(`http://servico-livros:3002/livros/${emprestimo.livro_id}`);
        const livro = await livroResponse.json();
        
        await fetch(`http://servico-livros:3002/livros/${emprestimo.livro_id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ copias_disponiveis: livro.copias_disponiveis + 1 })
        });
        ...
        const dataPrevista = new Date(devolucao[0].data_prevista_devolucao);
        ...
        if (dataDevolucao > dataPrevista) {
          ...
          await fetch(`http://servico-multas:3004/multas`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              emprestimoId,
    ...
    async criarReserva({ usuarioId, livroId }) {
      ...
      try {
        ...
        // Verificar livro
        const livroResponse = await fetch(`http://servico-livros:3002/livros/${livroId}`);
        if (!livroResponse.ok) {
          throw NotFoundError("Livro não encontrado", "BOOK_NOT_FOUND");
        }
