export default function createLivrosService(db) {
  ...
  async function criarLivro(data) {
    ...
    // Verificar se usuário existe e é bibliotecário (chamada HTTP)
    if (criado_por) {
      try {
        const response = await fetch(`http://servico-usuarios:3001/usuarios/${criado_por}`);
        ...
        throw new Error("Erro ao verificar usuário criador");
      }
    }
  ...
  async function removerLivro(id) {
    ...
    try {
      const response = await fetch(`http://servico-emprestimos:3003/emprestimos?livroId=${id}&status=ativo`);
